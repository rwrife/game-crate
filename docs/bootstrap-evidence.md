...
- `GameCrate.xcodeproj` with shared `GameCrate` scheme (app + UI-test targets),
...
- `App/`: SwiftUI launch-only placeholder (`GameCrateApp` + `BootstrapHomeView`)
  wired to `Packages/CrateKit`.
- `Packages/CrateKit`: pure Swift 6 package (Foundation only) with
  swift-testing placeholder tests proving the Linux lane works.
- `UITests/GameCrateLaunchTests.swift`: simulator launch smoke test asserting
...
`GameCrate.xcodeproj` is exactly `1` (any `1,2` or `2` fails), and after the build the
`device_family_guard` phase converts the built `GameCrate.app/Info.plist` to JSON and
...
## Verification actually performed (Linux executor, 2026-09-24, no Swift/Xcode on host)

- `python3 -m unittest discover -s Scripts/tests` — 21 tests OK (helper tests for
  bounded boot/timeout/exit-code behavior, simulator selection, and exact Xcode
  pin selection).
- `swift test` for `Packages/CrateKit` executed for real inside Docker
  (`swift:6.2-noble`, matching the CI Linux job image): 3 tests in 1 suite passed.
...
- pbxproj object-ID closure probe (35 defined == 35 referenced, all 24-hex) — clean.
...
- grep: `TARGETED_DEVICE_FAMILY = 1;` present in all four target configurations,
  zero iPad variants, bundle id `com.infinityball.gamecrate` in both app configs.
